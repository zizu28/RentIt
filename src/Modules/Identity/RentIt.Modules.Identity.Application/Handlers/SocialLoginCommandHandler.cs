...
public sealed class SocialLoginCommandHandler(
    ...
    IUnitOfWork unitOfWork) : IRequestHandler<SocialLoginCommand, Result<LoginResponse>>
{
    ...
    public async Task<Result<LoginResponse>> Handle(SocialLoginCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (user == null)
            {
                ...
                // Generate a random strong password for social logins so they can't login via email/password easily, 
                // or they can reset it later.
